...
function showHelp(){
    ...
    echo "Example: ./timed_roslaunch.sh 2 turtlebot_navigation amcl_demo.launch initial_pose_x:=1.539 initial_pose_y:=2.8"
    echo
    ...
    echo
    echo '<launch>'
    echo '  <arg name="initial_pose_y" default="2.8" />'
    echo '  <node pkg="semantic_turtle_test" type="timed_roslaunch.sh"'
    echo '    args="2 turtlebot_navigation amcl_demo.launch initial_pose_x:=1.539 initial_pose_y:=$(arg initial_pose_y)"'
